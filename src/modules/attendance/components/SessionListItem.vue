...
<template>
  <button type="button" @click="emit('select', props.session.id)" :class="[
    'cursor-pointer! w-full text-left rounded-lg border px-4 py-3 transition-all',
    props.active
    ...
    <div class="flex items-start gap-3">
      <div
        :class="['mt-0.5 shrink-0 rounded-md p-2', props.active ? 'bg-indigo-100 text-indigo-600' : 'bg-zinc-100 text-zinc-500']">
      ...
      <div class="min-w-0 flex-1">
        ...
        <div class="flex items-center gap-3 mt-1.5 text-xs text-zinc-400">
          ...
          <span><i class="pi pi-clock mr-1" />{{ formatTime(props.session.starts_at) }} – {{
            formatTime(props.session.ends_at) }}</span>
        </div>
      </div>
      <span class="text-xs text-zinc-400 shrink-0">{{ props.session.scheduled_hours }}h</span>
    </div>
  </button>
</template>
