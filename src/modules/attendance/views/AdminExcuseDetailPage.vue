  ...
  <div class="max-w-2xl mx-auto px-4 py-8 space-y-6">
    ...
    <template v-else-if="excuse">
      ...
      <div class="rounded-xl border border-zinc-200 bg-white divide-y divide-zinc-100">
        <div class="px-5 py-4 grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <p class="text-sm font-medium text-zinc-800">
              {{ excuse.engagement.name }}
            </p>
            <p class="text-xs text-zinc-400 mt-0.5 capitalize" v-if="excuse.engagement.type">
              {{ excuse.engagement.type.replace('_', ' ') }}
            </p>
            <p class="text-xs text-zinc-400 mt-0.5">{{ formatDate(excuse.engagement.starts_at) }}</p>
            <p v-if="excuse.engagement.instructor" class="text-xs text-zinc-400 mt-0.5">
              <i class="pi pi-user mr-1" />{{ excuse.engagement.instructor }}
            </p>
          </div>
        </div>
        <div class="px-5 py-4">
        ...
        <div v-if="excuse.attachment_url" class="px-5 py-4 space-y-3">
          <p class="text-xs text-zinc-400">Attachment</p>

          <img v-if="/\.(jpg|jpeg|png|webp)$/i.test(excuse.attachment_url)" :src="excuse.attachment_url"
            class="max-h-64 rounded-lg border border-zinc-200 object-contain" alt="Excuse attachment" />

          <iframe v-else-if="/\.pdf$/i.test(excuse.attachment_url)" :src="excuse.attachment_url"
            class="w-full h-96 rounded-lg border border-zinc-200" />

          <a :href="excuse.attachment_url" target="_blank" rel="noopener noreferrer" class="cursor-pointer inline-flex items-center gap-1.5 text-sm text-indigo-600 hover:text-indigo-800
          hover:underline transition">
            <i class="pi pi-external-link" /> Open in new tab
          </a>
        </div>

        <div v-if="excuse.review" class="px-5 py-4 bg-zinc-50 rounded-b-xl">
          ...
          <p v-if="excuse.review.note" class="text-sm text-zinc-500 mt-1 italic">
            {{ excuse.review.note }}
          </p>
        </div>
      </div>
      ...
      <div v-else class="rounded-xl border border-zinc-100 bg-zinc-50 p-4 text-center text-sm text-zinc-400">
