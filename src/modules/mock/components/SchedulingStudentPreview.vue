...
const studentSchedule = ref<StudentTimelineSession[]>([
  {
    ...
    title: 'Laravel Core Architecture: Service Containers & Providers',
    instructor: 'Prof. Alan Turing',
    type: 'Lecture',
    location: 'Auditorium Main A',
    hours: 3,
    status: 'Upcoming',
  },
  {
    ...
    title: 'Vue.js Advanced State Management & Component Composition',
    instructor: 'Dr. Grace Hopper',
    type: 'Lab',
    location: 'Computing Laboratory 04',
    hours: 2,
    status: 'Upcoming',
  },
  {
    id: 3,
    timeBlock: '15:30 - 16:30',
    title: 'Individual Capstone Milestone Review Block',
    instructor: 'Prof. Alan Turing',
    type: 'Review',
    location: 'Office Suite 202',
    hours: 1,
    status: 'Upcoming',
  },
])
...
const getTypeBadgeClass = (type: StudentTimelineSession['type']) => {
  switch (type) {
    case 'Lecture':
      return 'badge bg-primary/10 border-primary/20 text-primary font-medium text-[10px]'
    case 'Lab':
      return 'badge bg-success/10 border-success/20 text-success font-medium text-[10px]'
    case 'Review':
      return 'badge bg-warning/10 border-warning/20 text-surface-800 font-medium text-[10px]'
  }
}
</script>
    ...
    >
      <div>
        ...
        <p class="text-xs text-surface-500">
          Target Role Focus: Student Portal (Personal Progress & Track Schedule Deliverables)
        </p>
      </div>
      <span
    ...
    <div class="p-6">
      <div class="space-y-4">
        <h4 class="text-xs font-bold uppercase tracking-wider text-surface-500 px-1">
          Today's Academic Chronological Timeline
        </h4>
        ...
        <div class="relative border-l-2 border-surface-200 ml-4 pl-6 space-y-6">
          <div v-for="session in studentSchedule" :key="session.id" class="relative">
            <span
              class="absolute -left-[31px] top-1.5 bg-white border-2 border-primary w-4 h-4 rounded-full flex items-center justify-center shadow-2xs z-10"
            ></span>
            ...
            >
              <div class="space-y-1 min-w-0">
                <div class="flex flex-wrap items-center gap-2">
                  ...
                  <span class="text-[11px] text-surface-400 font-medium"
                    >({{ session.hours }} hrs delivered)</span
                  >
                </div>
                <h5
                ...
                <div class="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-surface-500">
                  <span class="flex items-center gap-1"
                    ><i class="pi pi-user text-[11px]"></i> {{ session.instructor }}</span
                  >
                  <span class="flex items-center gap-1"
                    ><i class="pi pi-map-marker text-[11px]"></i> {{ session.location }}</span
                  >
                </div>
              </div>
              ...
              <div class="shrink-0 self-start sm:self-center">
                ...
                >
                  <span class="w-1.5 h-1.5 rounded-full bg-primary animate-pulse"></span> Track
                  Active
                </span>
              </div>
            </div>
