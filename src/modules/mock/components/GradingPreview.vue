...
const studentGrades = ref<StudentGradeRow[]>([
  { id: 1, name: 'Jane Doe', currentGrade: 88, isOverridden: false },
  {
    ...
    currentGrade: 62,
    isOverridden: true,
    justification: 'Approved medical leave adjustment during midterms.',
  },
...
function commitOverrideChange() {
  if (!selectedStudent.value || tempGradeHolder.value === null || !overrideJustification.value)
    return
...
<template>
  <div class="card bg-white border border-surface-200 shadow-xs w-full overflow-hidden">
    ...
    >
      <div>
        ...
        <p class="text-xs text-surface-500">
          Target Role Focus: Track Admins (Rules/Weights) & Instructors (Input/Overrides)
        </p>
      </div>
      <span
        class="text-[10px] font-mono bg-warning/10 text-warning-content border border-warning/20 px-2 py-0.5 rounded"
      >
    ...
    <div class="p-6 space-y-6">
      <div class="p-4 border border-surface-200 rounded-xl bg-surface-50/30 space-y-4">
        <div class="flex justify-between items-center flex-wrap gap-2">
          <div>
            <h4 class="text-xs font-bold uppercase tracking-wider text-surface-700">
              Cohort Component Weighting Allocation
            </h4>
            <p class="text-[11px] text-surface-400">
              Total system sum configuration rules must equal exactly 100%
            </p>
          </div>
          <div
            :class="[
              'badge font-mono text-xs font-bold px-2 py-1',
              totalWeight === 100 ? 'bg-success text-white' : 'bg-danger text-white',
            ]"
          >
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          <div
            v-for="item in weightAssignments"
            :key="item.component"
            class="bg-white p-3 border border-surface-200 rounded-lg shadow-2xs flex flex-col justify-between gap-2"
          >
            <span class="text-xs font-semibold text-surface-700 block truncate">{{
              item.component
            }}</span>
          ...
          <DataTable :value="studentGrades" class="text-xs min-w-[650px]" responsiveLayout="scroll">
            ...
            <Column field="currentGrade" header="Current Composite Score">
              <template #body="slotProps">
                <span class="font-mono font-semibold text-sm text-surface-700"
                  >{{ slotProps.data.currentGrade }}%</span
                >
              </template>
            </Column>
            <Column header="System Audit Flag">
              <template #body="slotProps">
                <div v-if="slotProps.data.isOverridden" class="flex flex-col gap-0.5">
                  <span
                    class="badge bg-danger/10 border-danger/20 text-danger text-[10px] font-bold"
                    >Manual Override Active</span
                  >
                  <span
                    class="text-[10px] text-surface-400 italic truncate max-w-[250px]"
                    :title="slotProps.data.justification"
                  >
                    "{{ slotProps.data.justification }}"
                  </span>
                </div>
                <span v-else class="text-surface-400 italic text-[11px]"
                  >System Standard Calculation</span
                >
              </template>
            </Column>
            <Column header="Actions" class="text-right">
              <template #body="slotProps">
                <button
                  @click="openOverrideModal(slotProps.data)"
                  class="btn btn-xs btn-outline border-surface-200 hover:bg-surface-100 text-surface-700 normal-case font-medium"
                >
    ...
    <Dialog
      v-model:visible="isOverrideModalVisible"
      modal
      ...
      <div class="space-y-4 pt-2">
        <div class="p-3 bg-surface-50 rounded-lg border border-surface-200 text-xs space-y-1">
          <p class="text-surface-500 font-medium">
            ...
            <span class="text-surface-900 font-bold">{{ selectedStudent?.name }}</span>
          </p>
          <p class="text-surface-500 font-medium">
            Current Calculated Value:
            <span class="text-surface-900 font-mono font-bold"
              >{{ selectedStudent?.currentGrade }}%</span
            >
          </p>
        </div>
        ...
        <div class="flex flex-col gap-1">
          ...
          <Textarea
            ...
            rows="3"
            fluid
            placeholder="State explicit educational reasons or context validating this manual entry logic..."
            class="text-sm"
          />
        </div>
        ...
        <div class="flex justify-end gap-2 pt-2">
          <button
            @click="isOverrideModalVisible = false"
          ...
          <button
            ...
            :disabled="!overrideJustification || tempGradeHolder === null"
            class="btn btn-sm btn-primary text-white font-medium normal-case"
          >
