...
<template>
  <div class="card bg-white border border-surface-200 shadow-xs w-full overflow-hidden">
    ...
    >
      <div>
        <h3 class="font-bold text-surface-900 text-sm">
          Module 3A: Engagement Calendar (Admin Interface)
        </h3>
        <p class="text-xs text-surface-500">
          Target Role Focus: Track Admins (Session Booking, Collision Checks)
        </p>
      </div>
      <span
    ...
    <div class="p-6 space-y-6">
      <div
        class="p-4 rounded-xl border border-danger/20 bg-danger/5 text-danger flex items-start gap-3"
      >
        ...
        <div class="text-xs space-y-1">
          ...
          <p class="text-surface-600">
            Dr. Grace Hopper is already booked for
            <span class="font-semibold text-surface-900">Fullstack Laravel - Cohort 4</span> during
            the requested hours (13:00 - 16:00) on the selected calendar baseline block.
          </p>
        </div>
      </div>

      <div
        class="flex items-center justify-between p-4 border border-surface-200 rounded-lg bg-surface-50/30"
      >
        <div class="min-w-0">
          ...
          <p class="text-[11px] text-surface-400 truncate">
            Create, track, and assign distinct hourly engagement matrices
          </p>
        </div>
        <button
          @click="isBookingModalVisible = true"
          class="btn btn-sm btn-primary text-white normal-case font-medium shrink-0"
        >
      ...
      <div
        ...
        class="p-4 border border-surface-200 bg-white rounded-xl shadow-xs space-y-4 max-w-md mx-auto"
      >
        <div
          class="text-xs font-bold text-surface-800 border-b border-surface-100 pb-2 flex justify-between items-center"
        >
          <span>Configure Session Engagement</span>
          <button
            @click="isBookingModalVisible = false"
        ...
        <div class="space-y-3 text-xs">
          ...
          <button
            @click="isBookingModalVisible = false"
            class="btn btn-xs btn-primary w-full text-white mt-2"
          >
