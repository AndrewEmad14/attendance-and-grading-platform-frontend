...
const commitOverrideChange = async () => {
  ...
  try {
    await overrideSubmission(props.submissionId, newScore.value as number, overrideNote.value.trim())
  ...
  >
    <div class="space-y-4 pt-2">
      <div class="p-3 bg-surface-50 rounded-lg border border-surface-200 text-xs space-y-1">
        <p class="text-surface-500 font-medium">Target Student profile: <span class="text-surface-900 font-bold">{{ studentName }}</span></p>
        <p class="text-surface-500 font-medium">Deliverable: <span class="text-surface-900">{{ deliverableName }}</span></p>
        <p class="text-surface-500 font-medium">Current Calculated Value: <span class="text-surface-900 font-mono font-bold">{{ originalScore }}</span> / {{ maxScore }}</p>
      </div>
      ...
      <div class="flex flex-col gap-1">
        ...
        <Textarea v-model="overrideNote" rows="3" fluid placeholder="State explicit educational reasons or context validating this manual entry logic... (Min 10 chars)" class="text-sm" />
      </div>

      <div class="flex justify-end gap-2 pt-2">
        <button @click="close" :disabled="saving" class="btn btn-sm btn-ghost text-surface-500 font-medium normal-case">Cancel</button>
        <button @click="commitOverrideChange" :disabled="isSubmitDisabled" class="btn btn-sm btn-primary text-white font-medium normal-case flex items-center gap-2">
