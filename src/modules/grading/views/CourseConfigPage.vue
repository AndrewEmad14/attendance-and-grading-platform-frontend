...
function addDeliverable() {
  editingDelivs.value.push({
    name: '', type: 'lab', max_score: 100, course_weight: 0, due_date: ''
  })
}
...
async function saveWeights() {
  ...
  try {
    ...
    toast.add({ severity: 'success', summary: 'Success', detail: 'Course configuration saved', life: 3000 })
  } catch (err: any) {
...
async function handleAddCourse() {
  ...
  try {
    ...
    toast.add({ severity: 'error', summary: 'Error', detail: err.message || 'Failed to create', life: 3000 })
  } finally {
    saving.value = false
  }
}
  ...
  confirm.require({
    ...
    accept: async () => {
      try {
        ...
        toast.add({ severity: 'error', summary: 'Error', detail: err.message || 'Failed to delete course', life: 3000 })
      }
    }
  })
}
  ...
  <div class="h-full flex flex-col">
    ...
    <DashboardGrid variant="main-with-sidebar">
      ...
      <ContentCard title="Courses">
        ...
        <div v-if="store.error" class="px-4 py-2 bg-red-50 text-red-600 text-xs border border-red-100 rounded-lg mb-3">
          {{ store.error }}
        </div>

        <ul class="flex flex-col gap-1 max-h-[600px] overflow-y-auto pr-1">
          <li v-if="store.loading && store.courses.length === 0"
              class="p-4 text-center text-surface-500 text-sm">
            Loading...
          </li>
          <li v-else-if="!store.loading && filtered.length === 0"
              class="p-4 text-center text-surface-500 text-sm">
            No courses yet.
          </li>
          <li
            ...
            >
              ...
              <div class="flex items-center gap-2 ml-2 flex-shrink-0">
                ...
                <i
                  ...
                  :class="selectedCourse?.id === course.id ? 'text-primary-500' : 'text-surface-400'"
      ...
      <ContentCard 
        ...
        :subtitle="selectedCourse ? 'Configure deliverable weights. All weights must sum to exactly 100%.' : 'Choose from the list on the left'"
      >
        <div v-if="!selectedCourse" class="flex flex-col items-center justify-center text-surface-400 gap-3 py-12">
        ...
        <template v-else>
          <div class="space-y-4">
            ...
            >
              <div class="flex-1">
                <div class="flex items-center gap-2 mb-2">
                  <i
                    :class="weightValid ? 'pi pi-check-circle text-green-600' : 'pi pi-exclamation-circle text-red-500'"
                  ...
                  >
                    {{
                      weightValid ? 'Allocation complete'
                      : weightSum > 100 ? `Exceeds total by ${weightSum - 100}%`
              ...
              >
                <div class="grid grid-cols-12 gap-3 items-start">
                  ...
                  <div class="col-span-3">
                    <FormRow label="Name">
                      <InputText v-model="d.name" placeholder="e.g. Lab 1" size="small" class="w-full" />
                    </FormRow>
                  </div>

                  <div class="col-span-2">
                    <FormRow label="Type">
                      <Select v-model="d.type" :options="['lab', 'exam', 'project']" size="small" class="w-full" />
                    </FormRow>
                  </div>

                  <div class="col-span-2">
                    <FormRow label="Max Score">
                      <InputNumber v-model="d.max_score" :min="1" size="small" class="w-full" inputClass="w-full" />
                    </FormRow>
                  </div>

                  <div class="col-span-2">
                    <FormRow label="Weight %">
                      <InputNumber v-model="d.course_weight" :min="0" :max="100" suffix="%" size="small" class="w-full" inputClass="w-full" />
                    </FormRow>
                  </div>
                  ...
                  <div class="col-span-1 flex justify-center pt-6">
                    <button @click="removeDeliverable(i)" class="text-red-400 hover:text-red-600 transition-colors" title="Remove Deliverable">
            ...
            <div class="flex justify-end gap-2 pt-4 mt-4 border-t border-surface-200">
              ...
              <Button :label="saving ? 'Saving...' : 'Save Configuration'" icon="pi pi-save" size="small" :disabled="!weightValid || saving" :loading="saving" @click="saveWeights" />
            </div>
