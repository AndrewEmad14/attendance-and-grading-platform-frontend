...
<template>
  <div class="h-full flex flex-col">
    ...
    <DashboardGrid variant="main-with-sidebar">
      ...
      <ContentCard title="Courses">
        <template #headerAction>
          <div class="flex items-center gap-3">
            <select class="select select-bordered select-sm max-w-[220px] font-normal" :value="currentCohortId || ''"
        ...
        <div class="mb-3">
          <IconField>
            ...
            <InputText v-model="searchQuery" placeholder="Search courses..." size="small" class="w-full" />
          </IconField>
        </div>
        ...
        <ul class="flex flex-col gap-1 max-h-150 overflow-y-auto pr-1">
          <li v-if="store.loading && store.courses.length === 0" class="p-4 text-center text-surface-500 text-sm">
            Loading...
          </li>
          <li v-else-if="!store.loading && filtered.length === 0" class="p-4 text-center text-surface-500 text-sm">
            No courses yet.
          </li>
          <li v-for="course in filtered" :key="course.id" class="group">
            <button @click="selectCourse(course)"
              ...
              <div class="flex items-center gap-2 ml-2 flex-shrink-0">
                ...
                <i class="pi pi-chevron-right" :class="selectedCourse?.id === course.id ? 'text-primary-500' : 'text-surface-400'
      ...
      <!-- RIGHT: Weight Editor -->
      <ContentCard :isMainCanvas="true" :title="selectedCourse ? selectedCourse.name : 'Select a course'" :subtitle="selectedCourse
        ...
        <template v-else>
          <div class="space-y-4">
              ...
              <div class="flex-1">
                ...
                <div class="h-2 w-full rounded-full bg-white/70 overflow-hidden">
                  <div class="h-full rounded-full transition-all duration-300"
                    :class="weightValid ? 'bg-success-content' : 'bg-danger-content'"
            ...
            <!-- Deliverable Rows -->
            <div class="space-y-3">
              <div v-for="(d, i) in editingDelivs" :key="i"
                ...
                <div class="grid grid-cols-12 gap-3 items-start">
                  ...
                  <div class="col-span-2">
                    <FormRow label="Weight %">
                      <InputNumber v-model="d.course_weight" :min="0" :max="100" size="small" class="w-full"
                        inputClass="w-full" :disabled="isLocked || false" />
                    </FormRow>
                  </div>
