    ...
    <div v-else class="space-y-2">
      <div v-for="cohort in cohorts" :key="cohort.id"
        ...
        <div class="flex items-center gap-3 min-w-0">
          ...
          <span class="px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider" :class="cohort.is_active ? 'bg-success/10 text-success-content' : 'bg-surface-100 text-surface-500'
            ">
        ...
        <div class="flex items-center gap-2 shrink-0">
          <button @click="emit('manage-admins', cohort)"
          ...
          <button @click="emit('toggle-status', cohort)" :disabled="togglingId === cohort.id"
