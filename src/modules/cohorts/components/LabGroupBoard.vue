  ...
  <ContentCard title="Lab Groups"
    ...
    <div v-else class="grid grid-cols-1 xl:grid-cols-3 gap-4 items-start">
      ...
      <div class="rounded-lg border border-surface-200 bg-surface-50 overflow-hidden">
        <div class="px-3 py-2 border-b border-surface-200 bg-white flex items-center justify-between">
        ...
        <div v-if="unassignedStudents.length === 0" class="px-3 py-6 text-center text-xs text-surface-400 italic">
          Every student is in a group.
        </div>

        <ul v-else class="p-2 space-y-1.5 max-h-96 overflow-y-auto scrollbar-thin">
          <li v-for="student in unassignedStudents" :key="student.student_profile_id"
            ...
            <div v-if="labGroups.length" class="mt-2 flex flex-wrap gap-1">
              <button v-for="group in labGroups" :key="group.id" @click="
                ...
                <i v-if="mutatingStudentId === student.student_profile_id" class="pi pi-spin pi-spinner text-[9px]"></i>
      ...
      <!-- Group cards -->
      <div v-if="labGroups.length === 0" class="xl:col-span-2 py-8 text-center text-sm text-surface-400 italic">
        No lab groups yet. Create the first one to start assigning students.
      </div>

      <div v-for="group in labGroups" :key="group.id"
        class="rounded-lg border border-surface-200 bg-white overflow-hidden">
        <div class="px-3 py-2 border-b border-surface-200 bg-surface-50 flex items-center justify-between gap-2">
          <div class="min-w-0">
          ...
          <button @click="emit('delete-group', group)"
            class="text-danger-content cursor-pointer hover:bg-danger/5 rounded px-2 py-1 text-xs font-medium transition-colors flex items-center gap-1 shrink-0"
            title="Delete group">
        ...
        <ul v-else class="p-2 space-y-1.5 max-h-96 overflow-y-auto scrollbar-thin">
          <li v-for="student in group.students" :key="student.student_profile_id"
            class="flex items-center justify-between gap-2 rounded border border-surface-200 bg-white px-3 py-2">
            <div class="min-w-0">
              <div class="text-sm font-medium text-surface-900 truncate">{{ student.name }}</div>
              <div class="text-xs text-surface-500 truncate">{{ student.email }}</div>
            </div>
            <button @click="emit('drop', { labGroupId: group.id, studentId: student.student_profile_id })"
              :disabled="mutatingStudentId === student.student_profile_id"
              class="text-danger-content cursor-pointer hover:bg-danger/5 rounded px-2 py-1 text-xs font-medium transition-colors flex items-center gap-1 shrink-0 disabled:opacity-60">
              <i v-if="mutatingStudentId === student.student_profile_id" class="pi pi-spin pi-spinner text-[10px]"></i>
