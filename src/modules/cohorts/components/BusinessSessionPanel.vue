...
<template>
  <ContentCard title="Business Sessions"
    ...
    <template v-else>
      ...
      <div>
        ...
        <ul v-else class="space-y-1.5">
          <li v-for="session in attached" :key="session.id"
            ...
            <button @click="emit('detach', session)" :disabled="mutatingSessionId === session.id"
              class="text-danger-content cursor-pointer hover:bg-danger/5 rounded px-2 py-1 text-xs font-medium transition-colors flex items-center gap-1 disabled:opacity-60 shrink-0">
              <i v-if="mutatingSessionId === session.id" class="pi pi-spin pi-spinner text-[10px]"></i>
      ...
      <div class="pt-2 border-t border-surface-100">
        ...
        <ul v-else class="space-y-1.5 max-h-64 overflow-y-auto scrollbar-thin">
          <li v-for="session in available" :key="session.id"
            class="flex items-center justify-between gap-2 rounded border border-surface-200 bg-surface-50 px-3 py-2">
            <div class="text-sm font-medium text-surface-900 truncate">{{ session.name }}</div>
            <button @click="emit('attach', session)" :disabled="mutatingSessionId === session.id"
