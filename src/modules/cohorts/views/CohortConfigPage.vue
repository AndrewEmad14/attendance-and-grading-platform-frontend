...
async function executeCreateCohort() {
  ...
  try {
    ...
    const freshCohort = await cohortApi.createCohort(contextStore.selectedTrackId, { number: nextNumber, is_active: true })
...
async function executeCreateLabGroup() {
  ...
  try {
    ...
    const freshGroup = await cohortApi.createLabGroup(contextStore.selectedCohortId, { name: nextName })
...
async function executeEnrollBusinessSession() {
  ...
  isWorking.value = true
  try {
    await scheduleApi.enrollCohortInBusinessSession(selectedBusinessSessionId.value, contextStore.selectedCohortId)
    logOutput('enrollCohortInBusinessSession Success', { sessionId: selectedBusinessSessionId.value, cohortId: contextStore.selectedCohortId })
    await handleCohortChange()
  } catch (err: any) {
...
<template>
  <div class="p-2">
    <div class="border p-2 mb-2 flex items-center gap-4">
      ...
      <div v-if="isWorking" class="text-xs font-medium animate-pulse">Processing Network Layer Request...</div>
    </div>
        ...
        <div class="space-y-4">
          ...
          <div>
            ...
            >
              ...
              <option v-for="c in cohorts" :key="c.id" :value="c.id">Cohort Intake #{{ c.number }} (Active Status: {{ c.is_active }})</option>
            </select>
          </div>
      ...
      <ContentCard :isMainCanvas="true" title="Authorized Functional Action Execution Matrix">
        <div class="grid grid-cols-2 gap-4">
          ...
          <div class="space-y-4 border-r pr-4">
            <h3 class="text-xs font-bold uppercase tracking-wider text-gray-500">Write / Mutate Requests</h3>
            
            <div v-if="currentRole === 'branch_manager'" class="border p-2 space-y-2">
              <span class="text-xs block font-semibold text-gray-500">[Role Block: Branch Manager]</span>
              <button 
            ...
            <div v-if="currentRole === 'track_admin' || currentRole === 'branch_manager'" class="border p-2 space-y-2">
              <span class="text-xs block font-semibold text-gray-500">[Role Block: Track Admin]</span>
              ...
              <button 
                ...
                class="btn btn-outline btn-xs w-full text-gray-500"
              >
                POST /cohorts/{{ contextStore.selectedCohortId || '?' }}/lab-groups (Create Lab Group)
              </button>
              ...
              <div class="border p-1 space-y-1">
                ...
                <select v-model="selectedBusinessSessionId" class="select select-bordered select-xs w-full">
                  <option :value="null">-- Select Global Event --</option>
                  <option v-for="bs in businessSessions" :key="bs.id" :value="bs.id">{{ bs.name }}</option>
                </select>
                <button 
            ...
            <div v-else class="text-xs text-gray-400 italic">
              Simulated identity "{{ currentRole }}" holds no write access scopes for allocation modifications on this domain.
            </div>
          </div>

          <div class="space-y-4">
            <h3 class="text-xs font-bold uppercase tracking-wider  text-gray-500">Read Registers & Real-time Allocation Matrix</h3>

            <div>
              <h4 class="text-xs font-semibold mb-1 text-gray-500">Unassigned Cohort Pool (Students Count: {{ poolStudents.length }})</h4>
              <ul class="border max-h-32 overflow-y-auto p-1 text-xs space-y-1">
                <li v-if="poolStudents.length === 0" class="text-gray-400 italic">No unassigned pool entries parsed.</li>
                <li v-for="s in poolStudents" :key="s.student_profile_id" class="flex justify-between items-center border-b pb-1">
            ...
            <div>
              <h4 class="text-xs font-semibold mb-1 text-gray-500">Active Lab Sub-Divisions & Nested Rosters</h4>
              <div class="border max-h-40 overflow-y-auto p-1 text-xs space-y-2">
                <div v-if="labGroups.length === 0" class="text-gray-400 italic">No laboratory tracking matrix entities initialized.</div>
                <div v-for="g in labGroups" :key="g.id" class="border p-1 bg-gray-50">
                  <div class="font-bold border-b pb-0.5 mb-1 text-gray-500">{{ g.name }} (Students: {{ g.student_count ?? g.students?.length ?? 0 }})</div>
                  <ul class="space-y-0.5">
                    <li v-for="member in g.students" :key="member.student_profile_id" class="flex justify-between items-center bg-white p-0.5 pl-1">
            ...
            <div>
              <h4 class="text-xs font-semibold mb-1 text-gray-500">Active Timeline Booking References (Count: {{ engagements.length }})</h4>
              <ul class="border max-h-32 overflow-y-auto p-1 text-xs font-mono space-y-1">
                <li v-if="engagements.length === 0" class="text-gray-400 italic">No parsed scheduling tracks match criteria.</li>
                <li v-for="e in engagements" :key="e.id" class="border-b pb-0.5 text-gray-500">
                  {{ e.starts_at.substring(11,16) }} - {{ e.ends_at.substring(11,16) }} | {{ e.display_title }} ({{ e.display_context }})
                </li>
              </ul>
            </div>
        ...
        <div class="mt-4 border-t pt-2">
          <h3 class="text-xs font-bold mb-1 uppercase text-gray-500">Live Component Server Response Monitor</h3>
          <div class="bg-black text-green-400 font-mono text-[11px] p-2 rounded max-h-40 overflow-y-auto space-y-0.5">
            <div v-if="operationalLog.length === 0" class="text-gray-600 italic">Awaiting integration requests to pipeline tracking channels...</div>
            <div v-for="(entry, index) in operationalLog" :key="index" class="whitespace-pre-wrap leading-tight border-b border-gray-900 pb-0.5">
              {{ entry }}
            </div>
          </div>
        </div>
