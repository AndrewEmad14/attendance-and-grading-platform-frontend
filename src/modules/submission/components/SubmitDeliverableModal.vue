...
<template>
  <Dialog :visible="visible" @update:visible="emit('update:visible', $event)" modal
    :header="`Submit Deliverable: ${deliverable.name}`" :style="{ width: '480px' }" class="text-surface-800">
    <div class="space-y-4 pt-2">
      ...
      <div class="p-3 bg-surface-50 rounded-lg border border-surface-200 text-xs space-y-1">
        ...
        <p v-if="preview.isLate" class="font-medium text-warning-content flex items-center gap-1 pt-1">
      ...
      <div class="flex gap-1 p-1 bg-surface-100 rounded-lg">
        <button class="flex-1 py-1.5 rounded-md text-sm font-medium transition-colors normal-case cursor-pointer"
      ...
      <div v-if="mode === 'link'" class="flex flex-col gap-1">
        ...
        <input v-model="url" type="url" placeholder="https://github.com/you/project"
          class="input input-bordered bg-white border-surface-200 border-2 input-sm w-full" />
        <p v-if="url && !isLikelyUrl(url)" class="text-xs text-danger-content">
      ...
      <div v-else class="flex flex-col gap-1">
        ...
        <input type="file" :accept="ACCEPT_ATTR" class="bg-white file-input file-input-bordered file-input-sm w-full"
          @change="onFileChange" />
      ...
      <!-- Error -->
      <div v-if="errorMsg" class="p-2 bg-danger/10 text-danger text-xs rounded border border-danger/20">
        {{ errorMsg }}
      </div>

      <!-- Actions -->
      <div class="flex justify-end gap-2 pt-2">
        <button class="btn btn-sm btn-ghost text-surface-500 font-medium normal-case" :disabled="saving" @click="close">
          Cancel
        </button>
        <button class="btn btn-sm btn-primary text-white font-medium normal-case flex items-center gap-2"
